 ...
 *  */
$app->get('/chat_rooms/:id', function($mesaj_atan_id) {
    global $app;
    $db = new DbHandler();

    $result = $db->getChatRoom($mesaj_atan_id);
    ...
    while ($chat_room = $result->fetch_assoc()) {
        ...
        if ($i == 0) {
            $tmp = array();
            $tmp["chat_room_id"] = $chat_room["mesaj"];
            $tmp["name"] = $chat_room["mesaj_id"];
            $tmp["created_at"] = $chat_room["mesaj_atan_id"];
            $response['chat_room'] = $tmp;
        }

        if ($chat_room['mesaj_atan_id'] != NULL) {
            // message node
            $cmt = array();
            $cmt["message"] = $chat_room["mesaj"];
            $cmt["message_id"] = $chat_room["mesaj_zamani"];
            $cmt["created_at"] = $chat_room["mesaj_zamani"];

            // user node
            $user = array();
            $user['user_id'] = $chat_room['mesaj_zamani'];
            $user['username'] = $chat_room['mesaj_zamani'];
            $cmt['user'] = $user;
